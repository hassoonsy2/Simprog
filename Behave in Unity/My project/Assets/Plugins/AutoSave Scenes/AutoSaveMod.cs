...
namespace EM.AutoSave.Editor
{
	...
	internal class AutoSaveHelper
	{
		...
		{
			// get {
			if ( !System.IO.Directory.Exists( Application.dataPath + "/" + Root.AS_LOCATION ) )
			{
				System.IO.Directory.CreateDirectory( Application.dataPath + "/" + Root.AS_LOCATION );
				AssetDatabase.Refresh();
			}
			//if (!AssetDatabase.IsValidFolder("Assets/" + AutoSaveFolder)) AssetDatabase.CreateFolder("Assets", AutoSaveFolder);

			var files = System.IO.Directory.GetFiles(Application.dataPath + "/" + Root.AS_LOCATION).Select(f => f.Replace('\\', '/')).Where(f =>
			...
			var as_loc= Application.dataPath + "/" + Root.AS_LOCATION + "/";
	...
	class SETGUI_Autosave : UnityEditor.Editor
	{
		...
		public static void _GUI()
		{
			...
			{
				var res = EditorUtility.OpenFolderPanel( "Change Folder", Application.dataPath+ "/"+ Root.AS_LOCATION, "" );
				if ( !string.IsNullOrEmpty( res ) )
				{
					...
					if ( ValidateFolder( res ) )
					{
						...
						{
							Root.AS_LOCATION = res.Substring(Application.dataPath.Length).Replace( '\\', '/' ).Trim( '/' );
							AssetDatabase.Refresh();
						}
					}
				}
